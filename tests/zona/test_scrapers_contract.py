...
from scrapers.vivanuncios import parse_zona_html as vivanuncios_parse_zona_html

FIXTURES = Path(__file__).parent.parent / "fixtures"
FIXTURE = FIXTURES / "nuroa_zona.html"
FIXTURE_VIVANUNCIOS = FIXTURES / "vivanuncios_zona.html"
...
def test_vivanuncios_parse_zona_html_devuelve_registros_con_contrato():
    regs = vivanuncios_parse_zona_html(
        FIXTURE_VIVANUNCIOS.read_text(encoding="utf-8"), "departamento"
    )
    assert len(regs) >= 1
    r = regs[0]
    assert CLAVES.issubset(r.keys())
    assert r["fuente"] == "vivanuncios"
    assert r["tipo_inmueble"] == "departamento"
    assert isinstance(r["texto"], str) and r["texto"]
    # Vivanuncios deja el teléfono gateado (no se extrae).
